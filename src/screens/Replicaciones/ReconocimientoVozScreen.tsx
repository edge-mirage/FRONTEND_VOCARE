...
//   // --- MOCK: guardar y navegar al modal de confirmación
//   const finishRecording = async () => {
//     setIsRecording(false);
//     const mock = {
//       id: `rec_${Date.now()}`,
//       uri: `file:///mock/rec_${Date.now()}.m4a`,
//       durationSec: Math.min(seconds, 45),
//       createdAt: Date.now(),
//     };
//     try {
//       await AsyncStorage.setItem('replicacion:lastRecording', JSON.stringify(mock));
//     } catch {}

//     navigation.navigate('ReplicacionScreenSure', {
//       audio: { uri: mock.uri, durationSec: mock.durationSec, createdAt: mock.createdAt },
//     });
//   };
...
//         setSeconds(prev => {
//           if (prev >= 45) {
//             // llegó al tope: guardar y pasar al modal
//             clearInterval(timerRef.current!);
//             timerRef.current = null;
//             finishRecording();                // <-- NUEVO
//             return 45;
//           }
//           return prev + 1;
//         });
...
//   const toggleRecord = () => {
//     // TODO: pedir permisos y enganchar grabador real
//     if (isRecording) {
//       // si detiene manualmente, considera guardar si grabó algo
//       if (seconds > 0) finishRecording();     // <-- NUEVO (opcional)
//       else setIsRecording(false);
//       return;
//     }
//     if (seconds >= 45) setSeconds(0); // reinicia si ya llegó al límite
//     setIsRecording(true);
//   };
...
export default function ReconocimientoVozScreen() {
  ...
  const finishRecording = async () => {
    if (finishedRef.current) return; // evita duplicados
    ...
    try { await AsyncStorage.setItem('replicacion:lastRecording', JSON.stringify(mock)); } catch {}
  ...
  // Incrementa el contador mientras graba
  useEffect(() => {
    ...
    if (route.params?.reset) {
      ...
      setSeconds(0);
      setIsRecording(false);         
      navigation.setParams({ reset: undefined } as any); // limpia el flag
    }
  }, [route.params?.reset, navigation]);

  const toggleRecord = () => {
    // TODO: permisos + grabador real
    if (isRecording) {
      // si detiene manualmente y grabó algo, confirma de todos modos
      if (seconds > 0) finishRecording();
      else setIsRecording(false);
      return;
    }
    // iniciar
    finishedRef.current = false;
    if (seconds >= 45) setSeconds(0);
    setIsRecording(true);
  };
...
const styles = StyleSheet.create({
  ...
  bold: { fontWeight: '700' },
  whiteBox: {
    height: 200, borderRadius: 12, backgroundColor: colors.card ?? '#FFFFFF',
    marginHorizontal: spacing.sm, marginBottom: spacing.xl,
    elevation: 2, shadowColor: '#000', shadowOpacity: 0.07, shadowRadius: 6, shadowOffset: { width: 0, height: 3 },
  },
  micArea: { alignItems: 'center' },
  micButton: {
